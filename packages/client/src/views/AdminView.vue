...
function formatCachePct(n: number): string {
  if (!Number.isFinite(n) || n === 0) return '0%';
  // Keep one decimal when needed (e.g. 33.3%), otherwise whole number
...
async function loadCacheStats(): Promise<void> {
  try {
    ...
    if (response.ok) {
      ...
      if (data.last24h && typeof data.last24h === 'object') {
        cacheLast24h.value = {
          cacheHits: data.last24h.cacheHits ?? 0,
          generations: data.last24h.generations ?? 0,
          total: data.last24h.total ?? 0,
          cacheHitPercent: data.last24h.cacheHitPercent ?? 0,
          generationPercent: data.last24h.generationPercent ?? 0,
          windowHours: data.last24h.windowHours ?? 24,
        };
      }
    }
  } catch (error) {
          ...
          <div class="config-main">
            ...
            <div class="config-card">
              ...
              <p class="field-hint" style="margin-top: 0; margin-bottom: 16px;">
                ...
                Missing tracks still generate via AI and append to the same cache.
              </p>

              <div class="cache-stats">
                <div class="cache-stat">
                  <span class="cache-stat-label">Current entries</span>
                  <span class="cache-stat-value">{{ cacheEntryCount.toLocaleString() }}</span>
                </div>
                <div class="cache-stat">
                  <span class="cache-stat-label">Served from cache (24h)</span>
                  <span class="cache-stat-value">
                    {{ cacheLast24h.cacheHits.toLocaleString() }}
                    <span class="cache-stat-pct">({{ formatCachePct(cacheLast24h.cacheHitPercent) }})</span>
                  </span>
                </div>
                <div class="cache-stat">
                  <span class="cache-stat-label">Generated by AI (24h)</span>
                  <span class="cache-stat-value">
                    {{ cacheLast24h.generations.toLocaleString() }}
                    <span class="cache-stat-pct">({{ formatCachePct(cacheLast24h.generationPercent) }})</span>
                  </span>
                </div>
                <div class="cache-stat">
                  <span class="cache-stat-label">Total requests (24h)</span>
                  <span class="cache-stat-value">{{ cacheLast24h.total.toLocaleString() }}</span>
                </div>
              </div>
              <p class="field-hint" style="margin-top: 0; margin-bottom: 16px;">
                Last 24 hours counts each time a display asks for facts: cache hit vs first-time AI generation.
                Percentages are of total requests in that window
                <template v-if="cacheLast24h.total === 0"> (no requests yet since this version / restart window)</template>.
              </p>
